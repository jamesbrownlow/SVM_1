...
# SVM example- Regression included

get the library with function 'svm'
```{r}
library(e1071)
```
...
trainIndex = sample(1:150, 100, replace=FALSE) #set up the training sample
xTrain = x[trainIndex,]
yTrain = y[trainIndex]

model <- svm(xTrain, yTrain) # get the SVM model
...
xTest = x[-trainIndex,] # set up the training set
yTest = y[-trainIndex]
...
# regression model on two dimensions
...
points(x, new, col = 4)
legend('topleft', c('x', 'log (x)', 'predicted y'), col=c(1,2,4), lty=1)
