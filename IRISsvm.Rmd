...
## put the SVM in a loop;
first try 5 different samples

The idea is to run the 'svm' model 5 times, with a different sample each time, print the results

```{r}
nIris = 100 # put 100 observations in the training set

for (sampleRun in 1:5) {  # 5 iterations
  trainIndex = sample(1:150, 100, replace=FALSE) #get a new sample, size nIris
  
  xTrain = x[trainIndex,] # get new values for the training and test set
  yTrain = y[trainIndex]

model <- svm(xTrain, yTrain) # set up the model based on the new set
...
# Check accuracy:
(print(paste('random sample # ',sampleRun)))
print(table(pred, yTest))
